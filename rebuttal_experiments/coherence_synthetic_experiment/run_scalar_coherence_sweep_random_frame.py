...
def make_random_tight_embedding(
    ...
    for t in range(active_copies):
        ...
        basis[int(perm[2 * t].item())] = row_a
        basis[int(perm[2 * t + 1].item())] = row_b
    return basis
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--tag", type=str, default="scalar_coherence_sweep_random_frame_fast")
...
def main() -> None:
    ...
    for outer_seed in cfg.outer_seeds or []:
        mask_gen = torch.Generator().manual_seed(10_000 + outer_seed)
        mask = torch.bernoulli(torch.full((cfg.width, ambient_dim(cfg.copies)), cfg.active_prob), generator=mask_gen)
        mask_hist = pattern_histogram(mask)

        data_gen = torch.Generator().manual_seed(20_000 + outer_seed)
        train_latent = torch.randn(cfg.n_train, 2, generator=data_gen)
        test_latent = torch.randn(cfg.n_test, 2, generator=data_gen)

        frame_gen = torch.Generator().manual_seed(30_000 + outer_seed)
        perm = torch.randperm(ambient_dim(cfg.copies), generator=frame_gen)
        angles = torch.rand(cfg.copies, generator=frame_gen) * np.pi
        ...
        for active_copies in range(1, cfg.copies + 1):
            ...
            data = make_dataset_from_latent(cfg, train_latent, test_latent, basis)
            pair_records = []
            model_train_mses = []
            model_test_mses = []
            winner_records = []
    ...
    output = {
        "config": asdict(cfg),
        "embedding_family": "random_tight_frame",
        "anchor_aligned_endpoint": bool(args.anchor_aligned_endpoint),
